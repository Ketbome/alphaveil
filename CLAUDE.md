...
- `src/lib/worker.ts` — Web Worker: loads models, runs background removal (AutoModel + AutoProcessor, not the pipeline registry), tiled Swin2SR upscaling (`TILE` is 384: bigger tiles give the transformer more context and flatten textures less), EdgeTAM / SAM 2 (`samEmbed` once per image, `samMask` per click set and optional box; returns the three candidate masks with the best index) and ViTMatte refinement (`matte`: trimap from the current alpha, run at ≤1024 px, alpha resized back). Sessions are disposed, not just dropped: `release`/`dropCached` free their GPU buffers, `loadPipe` keeps only the bg and sr sessions in use, a run that dies on the WebGPU storage-buffer limit evicts its own session (`evict`) so the client can reload it on WASM, and one that loses the GPU device evicts every session (`evictAll`) since they all die with it.
- `src/lib/engine.ts` — promise-based client for the worker (progress/status callbacks). A worker that dies (`onerror` / `onmessageerror`) rejects everything in flight with an `EngineDead` error instead of leaving the UI waiting forever; `App` shows `t.errors.engineDied` for it.
...
- `src/lib/image.ts` — canvas helpers: file → bitmap, crop, trim, `smartCrop` (frame the subject), `composeBackdrop` (color / blurred photo + shadow), `sharpen` (unsharp mask: Swin2SR is a PSNR model, so its output is faithful but soft, and this is what puts the bite back; it leaves alpha alone and skips pixels whose neighbourhood is not fully opaque so cutouts get no halo), `exportBlob` with optional byte cap. `inspectAlpha` scans every pixel, so its answer is cached per bitmap in a `WeakMap`: bitmaps are immutable once created, and callers ask on every render — keep it that way.
...
- `src/lib/history.ts` — `trimHistory` caps a history by bytes (`MAX_BYTES`, per image) as well as by steps (`MAX_STEPS`), counting each buffer once since steps share their `origin`; the current step always survives. `Step { bitmap, kind, origin }`: `origin` is the original photo carried through the same crops, trims and upscales, so `compareBase()` always compares against the original, aligned. While only the framing changed, `origin` is the same object as `bitmap` and there is nothing to compare. `reframe()` in `App.tsx` is the only way to crop or trim.
- `src/App.tsx` — image queue (max 8, one active, history trimmed by `trimHistory`), toolbar, export, batch (`removeBgAll`, `downloadAll`). Upscaling stops at `MAX_SIDE_OUT` (8192 px a side): past that the buffers and the canvases behind them stop fitting in memory. `ImageQueue` floats over the preview canvas (thumbnails plus a ghost tile with `+`), it is not a row of its own.
...
- The preview renders the bitmap straight into a `<canvas>`. Thumbnails are box-filtered from the pixels into a 96 px canvas (never through a full-size one), and the crop dialog gets an object URL that `closeCrop` revokes — a data URL of a large image is a string as big as the image.
