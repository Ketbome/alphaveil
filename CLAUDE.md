...
- `src/lib/image.ts` — canvas helpers: file → bitmap, crop, trim, `smartCrop` (frame the subject), `composeBackdrop` (color / blurred photo + shadow), `exportBlob` with optional byte cap. `inspectAlpha` scans every pixel, so its answer is cached per bitmap in a `WeakMap`: bitmaps are immutable once created, and callers ask on every render — keep it that way.
